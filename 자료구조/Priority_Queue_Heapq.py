...
import heapq as hq # 여기서 heapq로 쓰기 귀찮다 하면 ✨as hq로 줄여서 사용할 수 있다.
import sys
...
# 백준 11286번 문제 절대값 힙 !! 
# 이문제에서는 n값이 최대 10만이므로 input()을 사용하면 너무 느리기 때문에 sys.readLine() 사용해서 푼다
# 변수안에 리드라인을 넣고 사용할때에는 readline ✨() 괄호를 넣으면 안된다. input에 넣고 사용하는건 그냥 사용자 스타일임
# abs()함수가 절대값을 찾는 함수이다 --> 음수를 넣으면 양수가 나오고 양수를 넣어도 양수가 나온다.
input = sys.stdin.readline 
test = []
for i in range (int(input())):
    x = int(input())
    if x != 0 :
        hq.heappush(test, (abs(x),x)) 
    else :
        print(hq.heappop(test)[1] if test else 0)
